        ...
        def update(self):
             ...
             if self.x_speed › 0: # идем направо, правый край персонажа - вплотную к левому краю стены
                 for p in platforms_touched:
                     self.rect.right = min (self-rect.right, p.rect.left) # если коснулись сразу нескольких, то правый кр
             elif self.x_speed ‹ 0: # идем налево, ставим левый край персонажа вплотную к правому краю стены
                 for p in platforms_touched:
                     self-rect.left = max(self.rect.left, p.rect.right) # если коснулись нескольких стен, то левый край
             ...
             if self-y_speed › 0: # идем вниз
                 for p in platforms_touched:
                     self-y_speed = 0
                     # Проверяем, какая из платформ снизу самая высокая, выравниваемся по ней, запоминаем её как свою от
                     if p.rect.top ‹ self.rect.bottom:
                         self.rect.bottom = p.rect.top
             elif self.y_speed < 0: # идем вверх
                 for p in platforms_touched: 
                     self-y_speed = ® # при столкновении со стеной вертикальная скорость гасится 
                     self.rect.top = max(self.rect.top, p.rect.bottom) # выравниваем верхний край по нижним краям стено:
...
cLass Enemy(GameSprite):
    ...
    def __init__(self, player_image, player_x, player_y, size_x, size_y, player_speed):
        # Вызываем конструктор класса (Sprite):
        GameSprite._ init_(self, player_image, player_x, player_y, size_x, size_y)
        self.speed = player_speed


    #движение врага
    def update self:
        if self.rect.x <= 420: #wI.walL_x + wi.wall_width
            self.side = "right"
...
back = (119, 210, 223)#задаем цвет согласно wdетовой схеме RGB
...
#cosdaem apynny ona MoHcmpo6
monsters = sprite.Group()


#создаем стены картинки
W1 = GameSprite( 'platform2.png' ,win width / 2 - win_width / 3,win_height / 2, 300, 50)
w2 = GameSprite('platform2_v.png', 370, 100, 50, 400)



#добавляем стены 6 группу
barriers. add (w1) 
barriers. add (w2)
...
monster1 = Enemy ('cyborg-png', win_width - 80, 150,
...
if sprite.spritecollide (packman, monsters,monsters, False):
finish = True
#бычисляем отношение
